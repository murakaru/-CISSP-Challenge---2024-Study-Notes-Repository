...
## Understand and integrate security in the Software Develoment Life Cycle
...
1. Requirements gathering: why create the software, what it will do, and for whom it will be created
2. Design: encapsulating how the software will meet requirements
3. Development: creating/coding the software to meet spec, and integrating with other systems as required
4. Testing: verifying/validating software meets requirements
5. Operations and Maintenance: deploying, and ensuring it's appropriately configured, patched, and monitored
6. Acceptance: formal, structured hand-off of the completed software system to the customer org; usually involves test, analysis and assessment activites
...
- Hierarchical database model: data elements and records are arranged in tree-like or parent-child structures
...
- Ransom attack:usually involves encrypting assets and withhold the decryption key until a ransom is paid by the victim. 
...
1. Scrum: a management framework that teams use to self-organize and work towards a common goal; it describes a set of meetings, tools, and roles for efficient project delivery, allowing teams to self-manage, learn from experience, and adapt to change; named from the daily team meetings, called scrums
2. Kanban: a visual system used to manage and keep track of work as it moves through a process; the word kanban is Japanese for "card you can see"; Kanban teams focus on reducing the time a project (or user story) takes from start to finish, using a kanban board and continuously improving their flow of work
3. Rapid Application Development (RAD): an agile software development approach that focuses more on ongoing software projects and user feedback and less on following a strict plan, emphasizing rapid prototyping over planning; RAD uses four phases: requirements planning, user design, construction, and cutover
4. Rational Unified Process (RUP): an agile software development methodology that splits the project life cycle into four phases:
...
5. Agile Unified Process (AUP): a simplified version of the rational unified process, it describes a simple, easy to understand approach to developing business application software using agile techniques and concepts yet still remaining true to the RUP
6. Dynamic Systems Development Model (DSDM): an agile project delivery framework, initially used as a software development method; key principles:
...
The waterfall model uses a linear sequential life-cycle approach where each phase must be completed before the next can begin; all project requirements are gathered up front, and there is no formal way to integrate changes as more information becomes available
...
improved waterfall dev process providing for a cycle of Plan, Do, Check, Act (PDCA) sub-stages at each phase of the SDLC; a risk-driven development process that follows an iterative model while also including waterfall elements
...
an approach to software development, quality assurance, and technology operations that seeks to unite siloed staff, and bring the three functions together in a single operational model
...
refers to the integration of development, security, and operations
...
a management process to foster the ongoing and continuous improvement of an org's processes and workflows for developing, maintaining and using software
...
CMM doesn't explicitly address security
...
developed by SEI, a model for software development that uses many of the SW-CMM attributes, using 5 phases:
...
Computers understand 1s and 0s (binary), and each CPU has its own (machine) language
    ...
    - Object-oriented programming (OOP): defines an object to be set of a software that offers one or more methods, internal to the object, that software external to that object can request to access; each method may require specific inputs and resources and may produce a specified set of outputs; focuses on the objects involved in an interaction. OOP languages include C++, Java, and .NET
...
### OOP terms:
        ...
        - behavior: results or output exhibited by an object. behaviors are the results of a message being processed through a method
...
Developers use a variety of tools, and one of the most important is the IDE 
...
**Static Application Security Testing (SAST)**:tools and technique to help identify software defects (e.g. data type errors, loop/structure bounds violations, unreachable code) or security policy violations and is carried out by examining the code without executing the program (or before the program is compiled)
...
**Dynamic Application Security Testing (DAST)** This is the evaluation of a program while running in real time
...
Open source is typically released with licensing allowing code access and inspection so devs can look for security issues. typically, however, this means that there is no sevice or support that comes with the software and requires in-house support for configuration, and security testing
...
Third-party software:  is software made specifically for an org by a third party
...
1. reducees the risk of a particular type of vuln
2. enforceable across all of an org's software development efforts
3. verifiably implemented
